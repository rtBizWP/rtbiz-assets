...
if ( ! class_exists( 'RT_Meta_Box_Assets_Info' ) ) {
	class RT_Meta_Box_Assets_Info {
		...
		public static function ui( $post ) {
			...
			<div class="row_group">
				<span class="prefix" title="<?php _e( 'Assigned To', RT_ASSET_TEXT_DOMAIN ); ?>"><label><strong><?php _e( 'Assigned To', RT_ASSET_TEXT_DOMAIN ); ?></strong></label></span>
				<input type="text" name="author" id="rt-asset-assignee" class="user-autocomplete" placeholder="Search for User" />
				<script>
				...
				<div id="selected_assignee">
					<?php
					if ( ! empty( $author ) ) { ?>
						<div id="rt-asset-assignee-<?php echo $author->ID; ?>"><?php echo get_avatar( $author->user_email, 25 ) . $author->display_name; ?>&nbsp;<a href="#removeAssignee">X</a><input type="hidden" name="post_author" value="<?php echo $author->ID; ?>" /></div>
					<?php } ?>
				</div>
			</div>
			...
			<div class="row_group">
				...
				<input class="datetimepicker moment-from-now" type="text" placeholder="Select Expiry Date"
				       ...
				       title="<?php echo esc_attr( ( isset( $expiry_date ) ) ? $expiry_date : '' ); ?>">
				<input
			...
			<div class="row_group">
				<span class="prefix" title="<?php _e( 'Vendor', RT_ASSET_TEXT_DOMAIN ); ?>"><label><strong><?php _e( 'Vendor', RT_ASSET_TEXT_DOMAIN ); ?></strong></label></span>
				<input type="text" name="vendor" id="rt-asset-vendor" class="user-autocomplete rt-asset-vendor" placeholder="Search for User" />
				<script>
					var arr_vendor_user =<?php echo json_encode( $arrVendorUser ); ?>;
				</script>
				<div id="selected_vendor">
					<?php
					if ( ! empty( $vendor ) ) { ?>
						<div id="rt-asset-vendor-<?php echo $vendor->ID; ?>"><?php echo get_avatar( $vendor->user_email, 25 ) . $vendor->display_name; ?>&nbsp;<a href="#removeVendor">X</a><input type="hidden" name="post[rtasset_vendor]" value="<?php echo $vendor->ID; ?>" /></div>
					<?php } ?>
				</div>
			</div>
